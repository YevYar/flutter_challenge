...
final _fetchMoreSuccessReducer = (MovieState state, FetchMoreSuccessAction action) =>
  state.copyWith(
    isFetchingMore: false,
...
final _fetchMoreFailedReducer = (MovieState state, FetchMoreFailedAction action) =>
  state.copyWith(
    isFetchingMore: false,
    searchErrorMessage: action.errorMessage
  );
...
final _fetchMovieSuccessReducer = (MovieState state, FetchMovieSuccessAction action) =>
  state.copyWith(
    isFetchingMovie: false,
    foundMovies: state.foundMovies.map(
      (item) => item.id == action.id ? item.copyWith(action.detailedInfo) : item
    ).toList(),
  );

final _fetchMovieFailedReducer = (MovieState state, FetchMovieFailedAction action) =>
  state.copyWith(
    isFetchingMovie: false
  );

final Reducer<MovieState> appReducer = combineReducers([
  ...
  new TypedReducer<MovieState, FetchMoreSuccessAction>(_fetchMoreSuccessReducer),
  ...
  new TypedReducer<MovieState, FetchMovieSuccessAction>(_fetchMovieSuccessReducer),
  new TypedReducer<MovieState, FetchMovieFailedAction>(_fetchMovieFailedReducer),
]);
